...
//left multiply vector(verticle vector)
...
void Matrix::Set(int x, int y, float v) {
    assert(x >= 0 && x < 4);
    assert(y >= 0 && y < 4);
    data[y][x] = v;
}
...
int Matrix::Inverse(float epsilon) { return Inverse(*this, epsilon); }

int Matrix::operator!=(const Matrix& m) const { return !(*this == m); }


Matrix operator*(float f, const Matrix& m) { return m * f; }
Matrix& Matrix::operator+=(const Matrix& m) { *this = *this + m; return *this; }
Matrix& Matrix::operator-=(const Matrix& m) { *this = *this - m; return *this; }
Matrix& Matrix::operator*=(const float f) { *this = *this * f; return *this; }
Matrix& Matrix::operator*=(const Matrix& m) { *this = *this * m; return *this; }

Matrix Matrix::MakeScale(float s) { return MakeScale(Vec3f(s, s, s)); }

void Matrix::Transform(Vec3f& v) const {
    Vec4f v2 = Vec4f(v.x(), v.y(), v.z(), 1);
    Transform(v2);
    v.Set(v2.x(), v2.y(), v2.z());
}

void Matrix::Transform(Vec2f& v) const {
    Vec4f v2 = Vec4f(v.x(), v.y(), 1, 1);
    Transform(v2);
    v.Set(v2.x(), v2.y());
}

void Matrix::TransformDirection(Vec3f& v) const {
